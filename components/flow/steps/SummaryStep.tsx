...
import { Platform, ScrollView, StyleSheet, TouchableOpacity, Alert, TextInput } from 'react-native';
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.header}>
           <Text style={[styles.mainTitle, { color: theme.text }]}>Resumen del Incidente</Text>
           <Text style={styles.subtitle}>Verificá los datos antes de enviar</Text>
        </View>
        ...
        <View style={[styles.emailSection, { backgroundColor: theme.card, borderColor: theme.border }]}>
          ...
          />
          <Text style={styles.emailHint}>Este es el destinatario que recibirá el informe completo con fotos.</Text>
        </View>
              ...
              <View key={party.id} style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
                <View style={styles.cardHeader}>
                  <Text style={styles.partyName}>
                    {party.useDniPhoto ? 'Identidad por foto' : `${party.name || 'Sin nombre'} ${party.surname || ''}`}
                  </Text>
      ...
      <View style={[styles.footer, { backgroundColor: theme.background }]}>
        ...
        >
          ...
          <Text style={[styles.outlineButtonText, { color: theme.text }]}>Enviar Informe</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  header: { paddingHorizontal: 24, marginTop: 20, marginBottom: 24, alignItems: 'flex-start' },
  mainTitle: { fontSize: 28, fontWeight: '900', textAlign: 'left' },
  subtitle: { fontSize: 15, opacity: 0.5, fontWeight: '600' },
  scroll: { flex: 1 },
  scrollContent: { paddingHorizontal: 24 },
  emailSection: { padding: 20, borderRadius: 24, borderWidth: 1, marginBottom: 24, gap: 12 },
  ...
  sectionTitle: { fontSize: 13, fontWeight: 'bold', opacity: 0.6, textTransform: 'uppercase', letterSpacing: 1 },
  card: { padding: 20, borderRadius: 24, borderWidth: 1, marginBottom: 12 },
  cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: '#00000005', paddingBottom: 12, marginBottom: 12 },
  ...
  infoLabel: { fontSize: 14, opacity: 0.5, fontWeight: '600', flex: 1, marginRight: 10 },
  infoValue: { fontSize: 14, fontWeight: '700', textAlign: 'right', flex: 1.5 },
  photoSummary: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 8, backgroundColor: '#00000005', padding: 8, borderRadius: 10, alignSelf: 'flex-start' },
  photoCount: { fontSize: 11, fontWeight: 'bold', opacity: 0.4 },
  footer: { padding: 24, paddingBottom: Platform.OS === 'ios' ? 40 : 24, gap: 12, borderTopWidth: 1, borderTopColor: '#00000010' },
  primaryButton: { padding: 20, borderRadius: 24, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 12 },
  primaryButtonText: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  outlineButton: { padding: 20, borderRadius: 24, borderWidth: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 12 },
